...
mod tests {
    ...
    fn registry_reconciles_authoritative_counts() {
        ...
        assert_eq!(
            registry.actions.iter().filter(|a| a.voice_eligible).count(),
            175
        );
        assert_eq!(
            registry
                .actions
                .iter()
                .flat_map(|action| action.utterances.iter())
                .count(),
            700
        );
        ...
        assert!(registry
            .actions
            .iter()
            .all(|action| action.required_slots.iter().all(|slot| slot.slot_id != "none")));
    }
    ...
    fn batch_b_planning_rows_are_overlaid_with_merged_evidence() {
        ...
        for action_id in [
            ...
            assert_eq!(
                action.evidence_state.as_deref(),
                Some("SBC7B1_BATCH_B_DUAL_PLATFORM_PASS")
            );
    ...
    fn convenience_aliases_canonicalise_with_fixed_recipe_facts() {
        ...
        let expected = [
            ...
            (
                "MONEY_OUT.PRIVATE_FROM_BUSINESS_FUNDS",
                "MONEY_OUT.SAVE",
                "business_use",
                "private",
            ),
    ...
    fn near_neighbour_open_books_is_ambiguous() {
        ...
        let outcome = controller.resolve(&ResolutionRequest {
            candidate_action_ids: vec![
                "BOOKS.OPEN".to_string(),
                "NAVIGATION.BOOKS_HOME".to_string(),
            ],
            facts: Vec::new(),
            invocation_source: InvocationSource::VoiceTranscript,
        });
    ...
    fn locked_action_is_known_but_not_executable() {
        let controller = controller(&[]);
        let outcome = controller.resolve(&ResolutionRequest {
            candidate_action_ids: vec!["OWNER.DRAWING.RECORD".to_string()],
            facts: Vec::new(),
    ...
    fn missing_context_is_distinct_from_owner_question() {
        let controller = controller(&["BOOKS.OPEN"]);
        let outcome = controller.resolve(&ResolutionRequest {
            candidate_action_ids: vec!["BOOKS.OPEN".to_string()],
            facts: Vec::new(),
            invocation_source: InvocationSource::CommandText,
        });
        assert_eq!(outcome.state, ResolutionState::Unknown);
    ...
    fn conflicting_owner_facts_fail_closed() {
        ...
        let outcome = controller.resolve(&ResolutionRequest {
            ...
            facts: vec![
                fact("contact_id", "contact-1"),
                fact("kind", "customer"),
                fact("kind", "supplier"),
                fact("display_name", "Example"),
            ],
            invocation_source: InvocationSource::VoiceTranscript,
        });
    ...
    fn alias_recipe_supplies_fixed_fact_without_new_authority() {
        ...
        assert!(outcome.resolved_facts.iter().any(|fact| {
            fact.slot_id == "kind" && fact.value == "customer" && fact.source == FactSource::AliasRecipe
        }));
    }
    ...
    fn confirmation_cannot_be_lowered_by_surface() {
        ...
        let facts = vec![
            fact("record_id", "expense-1"),
            fact("description", "Fuel"),
            fact("date", "2026-09-15"),
            fact("positive_gbp_pence", "4800"),
            fact("category", "MOTOR"),
            fact("business_use", "business"),
    ...
    fn confirmation_fails_when_state_or_facts_change() {
        ...
        let receipt = controller
            ...
            .expect("receipt");
        controller
            .validate_confirmation(&receipt, "revision-1", &facts)
            .expect("current");
        assert!(
            controller
                .validate_confirmation(&receipt, "revision-2", &facts)
                .is_err()
        );
        let changed = vec![
            fact("contact_id", "contact-1"),
        ...
        assert!(
            controller
                .validate_confirmation(&receipt, "revision-1", &changed)
                .is_err()
        );
    }
    ...
    fn replay_is_consumed_only_after_current_confirmation_passes() {
        ...
        let call = controller
            .prepare_call(
                &resolution,
                "operation-1",
                InvocationSource::VoiceTranscript,
                "revision-1",
            )
            .expect("call");
        let receipt = controller
        ...
        assert!(controller
            .accept_call(&call, "revision-1", None, &mut guard)
            .is_err());
        controller
            .accept_call(&call, "revision-1", Some(&receipt), &mut guard)
            .expect("confirmed first acceptance");
    ...
    fn stale_state_fails_before_replay_id_is_consumed() {
        ...
        let call = controller
            .prepare_call(&resolution, "report-op", InvocationSource::Manual, "revision-1")
            .expect("call");
        let mut guard = ReplayGuard::default();
        assert!(controller
            .accept_call(&call, "revision-2", None, &mut guard)
            .is_err());
        controller
            .accept_call(&call, "revision-1", None, &mut guard)
            .expect("fresh retry accepted");
    }
