...
RESULT_DIR="$ARTIFACTS/sbc7b1_mac_$TS"
...
finish() {
  ...
  cat > "$SUMMARY_TXT" <<EOF
SBC-7B1 Codemagic Mac/Apple Gate
Overall: $OVERALL
...
git -C "$REPO_ROOT" merge-base --is-ancestor "$BASE" "$HEAD" || stop "SBC-7A protected merge is candidate ancestor" "BASE_HEAD_NOT_ANCESTOR" 25
pass "SBC-7A protected merge is candidate ancestor"

PHASE="7B1_1_STATIC"
run_log static_gate python3 "$REPO_ROOT/scripts/check_sbc7b1_owner_bridge.py" --static-only || stop "SBC-7B1 static governance gate passes" "STATIC_GATE_FAIL" 30
pass "SBC-7B1 static governance gate passes"

LOCK_SHA="$(shasum -a 256 "$WORKSPACE/Cargo.lock" | awk '{print $1}')"
[ "$LOCK_SHA" = "$LOCK_EXPECTED" ] || stop "Native Cargo.lock equals reviewed SBC-7B1 hash" "LOCK_DRIFT" 31
pass "Native Cargo.lock equals reviewed SBC-7B1 hash"

PHASE="7B1_2_TOOLCHAIN"
...
pass "7B1 Tauri shell compiles for Apple Simulator target"
...
PHASE="COMPLETE"
OVERALL="PASS"
CLASSIFICATION="PASS_SBC7B1_MAC_APPLE"
pass "SBC-7B1 Mac/Apple bounded gate complete"
